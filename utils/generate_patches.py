...
if os.environ.get("MODEL_TYPE", None) is not None:
    from kv_caches import get_kv_cache
else:
    def get_kv_cache(x, y):
        return None
...
def _sample(
    ...
    **model_kwargs,
    ...
    while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
        ...
        if is_prefill:
            if getattr(kv_cache_criteria, "supports_online_prefill", False):
                model_inputs["kv_cache_criteria"] = kv_cache_criteria
                model_inputs["tgv_kv_input_ids"] = input_ids
            outputs = self(**model_inputs, return_dict=True, output_attentions=True)
            is_prefill = False
            num_of_token = cur_len
        else:
            outputs = model_forward(**model_inputs, return_dict=True, output_attentions=True)
            num_of_token += 1
        ...
        if kv_cache_criteria is not None:
            try:
                ...
                if hasattr(kv_cache_criteria, "pop_pending_attentions"):
                    pending_attentions = kv_cache_criteria.pop_pending_attentions()
                    if pending_attentions is not None:
                        attentions = pending_attentions
                past_key_values = kv_cache_criteria(
                ...
                print("Error in applying kv_cache_criteria:", e)
                pass
        ...
        if num_generated_tokens == max_generated_tokens:
            break
